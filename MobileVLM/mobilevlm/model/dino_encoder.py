...
class DinoVisionTower(BaseVisionTower):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        base_model_name, res, interp = 'dinov2-large',518,336
        ...
        if not self.delay_load:
            ...
            self.cfg_only = Dinov2Config.from_pretrained(self._vision_tower_name)

    def load_model(self, device_map=None):

        self.vision_tower = Dinov2Model.from_pretrained(self._vision_tower_name)
        ...
        print(self._vision_tower_name)
        # processor = AutoImageProcessor.from_pretrained(self._vision_tower_name, crop_size=dict(height=self._image_size, width=self._image_size), size=dict(shortest_edge=shortest_edge))
        
        # added by haoran use clip image processor
        # TODO: this is a temporary implement of clip processor
        processor = CLIPImageProcessor.from_pretrained("/model/BJiao/openaiclip-vit/openaiclip-vit-large-patch14-336")
        ############################################
    ...
    def feature_select(self, outputs):
        ...
        # added by haoran
        sequence_output = outputs[2][self.select_layer]
        
      
        # tensor2 = outputs[2][25+self.select_layer]
#         for i in range(25):
#             tensor2 = outputs[2][i]
#             # to verify 
